...
class Quitenicebooking_Prices_Daily {
	 ...
	 * @param int $type The accommodation ID
	 * @param array $entity_callback_args Parameters to pass to the callback
	 * @return float The price for the day
	 */
	public function calc_unit_price($date, $type, $entity_callback_args) {
		$total = 0.0;

		// don't load price rules if it's not per_person
		if (is_a($this->entity_scheme, 'Quitenicebooking_Entity_Per_person')) {
			$this->entity_scheme->load_price_rules($type, $date); // needs: accommodation id, date
		}
		...
		if (in_array(date('N', $date), array(5, 6, 7))) {
			...
			foreach (array_keys($this->entity_scheme->keys) as $entity_key) {
				...
				$entity_callback_args['daily_meta'] = 'weekday';
				// filter the base price if there's a filter
				if (has_filter('calc_unit_price')) {
					$base_price = apply_filters(
						'calc_unit_price', // hook
	...
	public function save_meta($id, $post) {
		foreach ($this->keys['composed'] as $key => $defs) {
			if (isset($post[$defs['meta_key']])) {
				 update_post_meta($id, $defs['meta_key'], sanitize_text_field(Quitenicebooking_Utilities::user_price_to_float($post[$defs['meta_key']], $this->settings)));
			}
		}
	}
}
