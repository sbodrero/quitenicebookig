...
<div class="booking-main-wrapper">
	...
	<?php if ( isset( $quitenicebooking_highlight ) && ! empty( $quitenicebooking_highlight ) ) { ?>
		...
		<div class="booking-main booking-main-highlight">
			...
			<h4 class="title-style4">
				<?php if ( $highlight_found ) { ?>
					<?php _e('Your Chosen Lodging Is Available','quitenicebooking'); ?><span class="title-block"></span>
				<?php } else { ?>
					<?php _e('Sorry, Your Chosen Lodging Is Unavailable','quitenicebooking'); ?><span class="title-block"></span>
				<?php } ?>
			</h4>
						...
						<div class="room-list-right">
							...
							<div class="room-price">
								<p class="price"><?php _e('From', 'quitenicebooking'); ?>: <span>
									<?php echo Quitenicebooking_Utilities::format_price(Quitenicebooking_Utilities::float_to_user_price($room['lowest_price'], $this->settings, TRUE), $this->settings,true); ?>
									</span> / <?php _e('Week', 'quitenicebooking'); ?> </p>
							...
							<?php foreach ($quitenicebooking_beds->keys['beds'] as $bed => $defs) { // display "Select <bed>" buttons ?>
								<?php if (isset($room[$defs['meta_key']]) && !empty($room[$defs['meta_key']])) { ?>
									<?php if ( $highlight_found ) { ?>
										<button type="submit" class="button2" name="room_<?php echo $quitenicebooking_room_number; ?>_selection" value="type=<?php echo $room['id']; ?>&bed=<?php echo $bed; ?>"><?php printf(_x('Select %s', 'Select bed/Lodging button', 'quitenicebooking'), $defs['description']); ?></button>
									<?php } else { ?>
										<span class="button2"><?php printf(_x('Select %s', 'Select bed/Lodging button', 'quitenicebooking'), $defs['description']); ?></span>
									<?php } ?>
									<?php $bed_type_count ++; ?>
								<?php } // end if ?>
							<?php } // end foreach ?>
							...
							<?php if ($bed_type_count == 0) { // display "Select Room" button ?>
								<?php if ( $highlight_found ) { ?>
									<button type="submit" class="button2" name="room_<?php echo $quitenicebooking_room_number; ?>_selection" value="type=<?php echo $room['id']; ?>&bed=0"><?php printf(_x('Select %s', 'Select bed/Lodging button', 'quitenicebooking'), $quitenicebooking_beds->keys['disabled']['description']); ?></button>
								<?php } else { ?>
									<span class="button2"><?php printf(_x('Select %s', 'Select bed/Lodging button', 'quitenicebooking'), $quitenicebooking_beds->keys['disabled']['description']); ?></span>
								<?php } ?>
							<?php } // end if ?>
							<!-- END .bookroom -->
	...
	<div class="booking-main">
		...
		<h4 class="title-style4">
			<?php if ( isset( $quitenicebooking_highlight ) && ! empty( $quitenicebooking_highlight ) ) { ?>
				<?php _e('Other Available Lodgings','quitenicebooking'); ?><span class="title-block"></span>
			<?php } else { ?>
				<?php _e('Choose Your Lodging','quitenicebooking'); ?><span class="title-block"></span>
			<?php } ?>
		</h4>
		
		<?php if (count($quitenicebooking_available_rooms) == 0) { ?>
			<p><?php _e('Sorry! There are no Lodgings available for these dates', 'quitenicebooking'); ?></p>
		<?php } else { ?>
...
<div class="booking-side-wrapper">
	...
	<div class="booking-side clearfix">
		...
		<?php } else { ?>
			<h4 class="title-style4"><?php printf(__('Lodging %d of %d', 'quitenicebooking'), $quitenicebooking_current_substep, $quitenicebooking_total_substeps); ?><span class="title-block"></span></h4>
		<?php } ?>
	...
	<?php if ($quitenicebooking_prev_room_number !== NULL) { ?>
		<div class="booking-side clearfix back-wrapper">
			<form action="<?php echo $quitenicebooking_step_2_url; ?>" method="POST">
				<input type="hidden" name="booking_step_2_submit" value="1">
				<button type="submit" class="button3" name="room_<?php echo $quitenicebooking_prev_room_number; ?>_selection" value=""><?php _e('&larr; Edit previous Lodging', 'quitenicebooking'); ?></button>
			</form>
		</div>
	<?php } // end if ?>
	<!-- END back-button -->
