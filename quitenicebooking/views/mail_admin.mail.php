...
<hr />
<?php if (!empty($data['booking_id'])) { ?><p><strong><?php _e('Booking ID', 'quitenicebooking'); ?>:</strong> <?php echo $data['booking_id']; ?></p><?php } ?>
<?php $n = 1; ?>
<?php foreach ($data['summary'] as $room) { ?>
<?php if (count($data['summary']) == 1) { ?>
<h4><?php _e('Lodging Info', 'quitenicebooking'); ?></h4>
<?php } else { ?>
<h4><?php printf(__('Lodging %d Info', 'quitenicebooking'), $n++); ?></h4>
<?php } ?>
<ul style="margin: 0px;padding:0 0 0 15px;">
<li style="margin-bottom: 3px;"><strong><?php _e('Lodging Type', 'quitenicebooking'); ?>:</strong> <?php echo $room['type']; ?></li>
...
<li style="margin-bottom: 3px;"><strong><?php _e('Services Requested', 'quitenicebooking'); ?>:</strong> <?php ob_start(); foreach ($data['services'] as $s) { echo $s['title'].', '; } echo substr(ob_get_clean(), 0, -2); ?></li>
<?php } ?>
...
<li style="margin-bottom: 3px;"><strong><?php _e('Deposit Due', 'quitenicebooking'); ?>:</strong> <?php echo Quitenicebooking_Utilities::format_price(Quitenicebooking_Utilities::float_to_user_price($data['deposit'], $this->settings, TRUE), $this->settings); ?></li>
<?php } // end if ?>
<li style="margin-bottom: 3px;"><strong><?php _e('Total Price', 'quitenicebooking'); ?>:</strong> <?php echo Quitenicebooking_Utilities::format_price(Quitenicebooking_Utilities::float_to_user_price($data['total'], $this->settings, TRUE), $this->settings); ?></li>
</ul>
</body>
</html>
